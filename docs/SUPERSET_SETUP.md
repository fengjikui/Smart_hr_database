...
**手工学习提示：本机已备份并清空当前演示配置，正在从零重建。** 下文描述完整配置的目标状态和自动化实现，不代表本机此刻已全部配置好。请按 [从零实操](HANDS_ON.md) 一步一步执行；不要现在运行自动初始化。

实操手册现已补全 45 步，包括完整样本导入、UI 配置、读取实际对象 ID、恢复 Agent 查询与撤权验收。`learning.py` 只生成材料与连接清单；保留你手工执行和验证的过程。
