# V2 完整数据接入 PostgreSQL 与 Superset：实施与复现讲义

实施日期：2026-09-20。环境：本机 Superset 6.1.0、PostgreSQL 17.11、原 V2 LangGraph Agent。

这份文档记录本项目实际实现的步骤。建议先读第 1～5 节，看懂数据、身份与权限；再读第 6～8 节理解 Agent 如何接入，最后按第 9 节手动验证。

## 1. 这次交付包含什么

原 V2 的完整 **300 人、26 个业务字段**已迁入新的 PostgreSQL 数据库 `hr_v2`。五个演示身份和原业务权限策略也已导入。原 SQLite 文件保留作独立回归参考，不再作为 Superset 模式的在线人员数据源。

应用会话、聊天历史和审计仍保存在应用 SQLite；这是应用状态，与人员业务数据分开。字段描述、指标口径仍由 Git 中的 Python 目录定义。此次没有迁移旧版 V1 的考勤表，也没有接入真实 HR 数据。
...
此前 `hr_lab` 数据库、`HR_LAB_*` 实验和你亲手填写的 `LEARN_*` 课堂对象继续保留。这次不通过重建容器或删除卷安装。
...
实际调用 [export.py](../integrations/superset/v2/export.py)：

1. 从 [store.py](../backend/hr/v2/store.py) 的 `people()` 读取当前 V2 宽表，而不是另造一套相似数据。
2. 读取同文件的 `PERSONAS` 和 `policy()`，获得五个身份及当前业务策略。
3. 从 [schema.py](../backend/hr/v2/schema.py) 读取字段 ID、字段组、描述。
4. 将全部人员、空值、字符串工号、快照日期、字段目录和策略写入导入快照。
5. 计算排序后的数据 SHA-256 指纹，导入和回归时用于核对内容。

输出在 `integrations/superset/.local/v2/fixtures.json`。这是可阅读 JSON，不含密码，未提交 Git。工号 `00031266` 始终作为字符串，不能转换为整数。
...
[run.py](../integrations/superset/v2/run.py) 先导出，再定位本项目已运行的 Superset 容器，执行 [setup.py](../integrations/superset/v2/setup.py)。它使用现有实验的 Docker socket，不修改全局 Docker context。
...
字段定义由导出文件驱动，主键和工号索引在 PostgreSQL 创建。日期保留 ISO 文本，年龄为整数，便于与现有 V2 口径逐项核对。安装程序使用参数化批量写入，不把姓名、学校等拼入 SQL。

首次安装在事务中写入完整人员与策略。再次执行 `setup` 会保留已有业务数据、用户角色和 RLS 编辑，并重新应用代码里的视图定义。因此它不是权限重置命令，也不是一个可以无审查覆盖自定义视图的生产发布工具。

只有明确需要以原 V2 文件覆盖 PostgreSQL 人员和业务策略时才运行：

```bash
uv run python integrations/superset/v2/run.py sync-data
```
...
递归和 HRBP 业务规则位于 [schema.sql](../integrations/superset/v2/schema.sql)：
...
若你手动复现，可用技术管理员进入 **Settings → Database Connections → + Database**，选择 PostgreSQL，填写数据库和对应只读账号。两个 PostgreSQL 账号的连接参数保存于本机 `integrations/superset/.local/v2/database-credentials.json`（0600）；这里的密码与 Superset 登录密码不同。安装程序直接完成了这一步，无需为了练习重复建立同名连接。若旧安装还没有这个文件，可运行下面命令，只导出本机连接资料，不修改数据库或权限：
...
本机凭据：`integrations/superset/.local/v2/credentials.json`，权限为 0600，已排除在 Git 之外。身份映射及实际数据集 ID：同目录 `manifest.json`，没有密码。演示采用后端固定的账号映射；生产需要接入企业 SSO，这次没有实现真实单点登录。

**当前身份选择器是演示入口，不验证操作者确实是这名员工。** 本机操作者能通过选择器及 `/api/v2/session` 切换五个 persona。会话绑定保证后续查询沿用已选身份，不能把它理解为已完成企业用户认证；上线前必须用可信登录身份替换这个入口。
...
手动查看/编辑角色请用 [经典角色列表](http://127.0.0.1:8088/roles/list/)。本机新版角色页面的权限搜索有兼容问题，经典入口已实测可用。数据集访问权限的实际名称随 ID 改变，可从 manifest 中查找；不要照抄旧课堂的 ID。
...
Base 的 Roles 表示豁免角色，不是需要受限制的角色；这次不给业务账号设置豁免。Clause 是 SQL 条件片段，不写完整 SELECT，也不加开头的 WHERE。
...
等价于给原启动器设置 `HR_V2_QUERY_BACKEND=superset`，仍然使用原 `/demo` 页面和 `/api/v2` API。常规 `npm run demo` 默认保留 SQLite 模式，用于旧基线对比。

主要文件与调用顺序：

1. [api.py](../backend/hr/v2/api.py)：读取 V2 会话与 CSRF，确定 persona；浏览器不能通过问题内容指定实际执行账号。
2. [auth.py](../backend/hr/v2/auth.py)：在 Superset 模式调用新的授权入口；原 Python 遍历只保留在 SQLite 基线模式和离线参考验证中。
3. [superset_source.py](../backend/hr/v2/superset_source.py)：用服务端清单将 persona 对应到独立 Superset 用户；登录获得临时令牌，读取受 RLS 限制的 context 与人员快照。
4. [registry.py](../backend/hr/v2/registry.py)：只给模型披露当前允许的字段、指标和部门候选。
5. [graph.py](../backend/hr/v2/graph.py)：原 LangGraph 继续完成检索、模型计划、口径校验、执行、解释和历史保存。
6. [query.py](../backend/hr/v2/query.py)：根据执行后端分派；Superset 模式交给 [superset_query.py](../backend/hr/v2/superset_query.py)。
...
9. [service.py](../backend/hr/v2/service.py)：返回数字摘要，保存查询与真实 SQL；结果返回和历史保存前重新检查授权指纹。
...
在 `/demo` 左下角展开“权限与模型状态”，可见“Superset 权限 · PostgreSQL 查询”。结果和独立节点调试页包含真实 PostgreSQL SQL，例如普通员工查询中应出现：
...
| 原界面的规则编辑 | Superset 模式停用，避免只改 SQLite 却误认为新链路生效 |
...
```bash
# 数据内容、数据库物理边界与Superset配置盘点
uv run python integrations/superset/v2/run.py verify-storage

# 真实Superset查询、20题结果、五身份与完整API功能
uv run python scripts/validate_v2_superset.py

# 短暂撤权/改关系/RLS，再自动恢复；不要与演示并行运行
uv run python scripts/validate_v2_superset_revocation.py

# 编译器与失败关闭单元测试
uv run pytest tests/test_v2_superset_query.py tests/test_v2_superset_source.py -q
```

本机已通过：51 项存储/配置检查、146 项集成检查、28 项可逆撤权检查。报告分别为 [存储与配置](../reports/v2-superset-storage.json)、[完整查询与接口](../reports/v2-superset-integration.json)、[撤权与恢复](../reports/v2-superset-revocation.json)。其中 LangGraph 检查固定了模型计划以稳定验证执行链路，不把它冒称为真实模型评测。

撤权覆盖了跨数据集情况：HR 的合同快照不变但公共 RLS 收窄、单独撤销事件数据集访问、只收窄事件 RLS，旧历史均不能继续使用。配置和关系在测试后精确恢复。

真实本地模型 `hr-qwen`（页面显示 Qwen3.8 · 27B）另外执行了 HR-01 管理线分组、HR-05 入离职事件、HR-16 合同到期三道题：**3/3 通过**，每题一次模型计划调用，耗时分别约 7.38、9.28、11.16 秒，完整结果与独立参考计算一致。见 [真实模型报告](../reports/v2-superset-model-smoke.json)。复现命令：
...
浏览器也验证了王灏的两轮提问：全部授权在职 169 人，继续追问可信与 AI 实验室得到 57 人；“独立对账”差异为 0，独立调试页能显示真实 SQL 的 `_viewer_id` 与部门筛选。这些检查不等于重新跑完 20 题真实模型评测；20 题完整覆盖的是确定计划的查询回归。

最终版本在浏览器切到员工冯基魁，提问“请列出我本人的姓名、部门和工号”，仅返回本人一行，工号 `00031266` 的前导零保留。全库 280 项 Python 测试、TypeScript 类型检查、前端 lint、Python Ruff 和语义文档一致性检查均通过。

GitHub Actions 已增加 V2 初始化、存储校验、集成查询及撤权恢复步骤。本次验证在本机完成，尚未运行远端 CI；CI 不依赖本机 LM Studio，真实模型验证单独执行。

撤权脚本只修改本次 V2 的对象，在 `finally` 中恢复，并使用临时会话/历史库。若操作系统强行终止测试，恢复快照位于本机 `.local/v2/probe-recovery.json`，需要先恢复，再继续演示。
...
7. 在 Agent `/demo` 选择王灏，先问“我的全部授权范围有多少在职员工”，再问“可信与AI实验室有多少在职员工”，对比 169 与 57。
...
| 聊天历史、会话、审计 | 原应用的 `v2_app.sqlite` |
| 本地密码及动态 ID 清单 | 被 Git 忽略的 `.local/v2/` |
...
这次交付证明的是：完整 V2 样本可以通过可配置的 Superset 访问控制接入原 Agent，并保留原指标语义与查询功能。真实企业上线仍需确认正式业务规则、可信 SSO、权限发布/同步、连接与模板管理、负载容量和运维要求。
