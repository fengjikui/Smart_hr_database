...
当前为 **21 张业务表、8 个应用逻辑表/全文索引、157 个字段、14 个指标**。字段数按各表列数相加，同名关联键分别计数。
...
| 层 | 位置 | 职责 |
|---|---|---|
| 业务库 | `data/hr.sqlite` | 员工、任职、组织、考勤等21张关系表；查询服务只读。 |
...
| business / `employees` | 人员基础档案；每名员工一行 | `id`, `employee_no`, `name`, `gender`, `birth_date`, `hire_date`, `termination_date`, `employment_type`, `entity_id`, `location_id`, `email` |
...
| business / `overtime_requests` | 加班申请；每人每日最多一行，与晚离岗分开 | `id`, `employee_id`, `day`, `minutes`, `approval_status`, `approver_id` |
...
目录版本 `hr-metrics-1.0`；下列表达式从实际编译的 SQL 提取。表达式依赖同一 SQL 的授权集合、日期与任职 CTE，不能脱离这些条件执行。完整示例计划及 SQL 可在系统中展开，也保存在同目录 `data-dictionary.json` 中。
...
- 单位：人；业务域：人员。
- 可分组：事业部 (`division`), 部门 (`department`), 岗位序列 (`job_family`), 工作地 (`location`), 用工类型 (`employment_type`), 汇报关系 (`relation`), 月份 (`month`)；也可不分组，单次只支持一个分组维度。
- 数据来源：employees, assignments。
- 别名：人数, 员工数, 人员规模, 下属。
- 敏感级别：`internal`；最小组规模：1。
- 责任人：HR 数据负责人（模拟）；指标版本：1.0。
- 示例问题：按事业部统计本月在职人数
...
- 单位：人；业务域：人员。
- 可分组：事业部 (`division`), 部门 (`department`), 工作地 (`location`), 月份 (`month`)；也可不分组，单次只支持一个分组维度。
- 数据来源：employees, assignments。
- 别名：新增, 入职, 招聘入职。
- 敏感级别：`internal`；最小组规模：1。
- 责任人：HR 数据负责人（模拟）；指标版本：1.0。
...
- 单位：人；业务域：人员。
- 可分组：事业部 (`division`), 部门 (`department`), 月份 (`month`)；也可不分组，单次只支持一个分组维度。
- 数据来源：employees, assignments。
- 别名：离职, 流失人数。
- 敏感级别：`internal`；最小组规模：1。
- 责任人：HR 数据负责人（模拟）；指标版本：1.0。
...
- 单位：%；业务域：人员。
- 可分组：事业部 (`division`), 部门 (`department`)；也可不分组，单次只支持一个分组维度。
- 数据来源：employees, assignments。
- 别名：离职率, 流失率。
- 敏感级别：`internal`；最小组规模：1。
- 责任人：HR 数据负责人（模拟）；指标版本：1.0。
...
- 单位：年；业务域：人员。
- 可分组：事业部 (`division`), 部门 (`department`), 岗位序列 (`job_family`)；也可不分组，单次只支持一个分组维度。
- 数据来源：employees, assignments。
- 别名：司龄, 工龄。
- 敏感级别：`internal`；最小组规模：1。
- 责任人：HR 数据负责人（模拟）；指标版本：1.0。
...
- 单位：%；业务域：考勤。
- 可分组：事业部 (`division`), 部门 (`department`), 日期 (`day`), 月份 (`month`)；也可不分组，单次只支持一个分组维度。
...
仅周一至周五模拟日历；整日请假；未实现节假日调休、半天假、跨夜班。正常/远程算完成出勤，缺卡与缺勤不算。
...
单指标、单维度。明细1–100行；聚合最多400组；日期差不超过366天。当前不支持部门数量、离职人员日期明细、年龄/性别/学历筛选、多指标、排名、同比环比、预测。
...
绩效评价、培训、招聘需求已有合成关系表，尚无对应问数指标；年龄、性别、学历等筛选未开放。私人电话、证件和银行账号不进入模型工具或查询白名单。基本月薪不是完整工资支付流水，目前没有奖金、补贴、扣款、社保、公积金、实发工资的计算与支付模型。
