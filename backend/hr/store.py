"""当前 SQLite 基线样本与应用状态存储，不是所有查询后端的统一业务数据库。

people.sqlite 保存原始合成宽表，供本地模式、迁移导出与离线参考使用；
sessions.sqlite 保存会话、历史、策略和审计。Superset 模式仍保留应用状态，
...
AS_OF = "2026-09-11"
# 已持久化的兼容标识；目录整理不改变样本或语义内容，因此保留原值。
...
def connection(kind="app", readonly=False):
    ...
    path = directory() / "people.sqlite" if kind == "people" else config.APP_DB
...
def ensure():
    ...
    with LOCK:
        ...
        if not (directory() / "people.sqlite").exists():
