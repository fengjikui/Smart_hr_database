...
async def model_status():
    try:
        ...
        return {'connected':config.MODEL_ID in ids,'model':config.MODEL_ID,'display_name':'Qwen3.8 · 27B','provider':'LM Studio','local':True,'message':'本地模型接口可用' if config.MODEL_ID in ids else '目标模型尚未加载'}
    except (httpx.HTTPError,ValueError,KeyError):
        return {'connected':False,'model':config.MODEL_ID,'display_name':'Qwen3.8 · 27B','provider':'LM Studio','local':True,'message':'LM Studio 服务未连接，请启动本地服务和模型。'}


def instructions(principal,as_of,previous=None):
    metrics=[{k:m[k] for k in ['id','name','description','dimensions']} for m in visible_catalog(principal)]
    with business() as db:
        ids=scope_ids(principal)
        marks=','.join('?' for _ in ids) or 'NULL'
        depts=[r[0] for r in db.execute(f'SELECT DISTINCT d.name FROM departments d JOIN assignments a ON a.department_id=d.id WHERE a.valid_to IS NULL AND a.employee_id IN ({marks})',ids)]
...
async def ask_model(messages):
    payload={'model':config.MODEL_ID,'messages':messages,'temperature':0,'max_tokens':750,'chat_template_kwargs':{'enable_thinking':False},'response_format':{'type':'json_schema','json_schema':{'name':'hr_query_plan','schema':QueryPlan.model_json_schema()}}}
...
async def answer(principal,question,previous_id=None):
    ...
    if previous_id:
        with application() as db:
            row=db.execute('SELECT plan FROM conversations WHERE id=? AND principal_id=? AND outcome=?',(previous_id,principal['id'],'success')).fetchone()
        if not row:
            raise HTTPException(404,detail='前次查询不存在或不属于当前身份。请重新描述问题。')
        previous=json.loads(row[0])
    messages=[{'role':'system','content':instructions(principal,as_of,previous)},{'role':'user','content':question}]
    repaired=False
    async with _gate:
        try:
            try:
                ...
                repaired=True
                messages.append({'role':'user','content':'上次输出未通过结构校验。请仅输出完整合法JSON，日期不明确时留空，禁止编造字段。'})
                plan,usage=await ask_model(messages)
            ...
            raise HTTPException(503,detail='本地模型暂时不可用或未产生有效计划。未执行数据库查询，请检查 LM Studio 后重试。') from exc
    model_ms=round((time.perf_counter()-started)*1000,2)
    trace=[{'name':'身份与范围','detail':'服务端读取当前授权','duration_ms':0},{'name':'语义规划','detail':'LM Studio · Qwen3.8 27B'+(' · 校验后重试1次' if repaired else ''),'duration_ms':model_ms}]
    if plan.kind in ('clarify','refuse'):
        output={'status':plan.kind,'summary':plan.message or ('请明确希望查看的指标或时间范围。' if plan.kind=='clarify' else '该请求不属于当前开放的查询能力。'),'plan':plan.model_dump(),'rows':[],'columns':[],'trace':trace,'model':config.MODEL_ID,'model_ms':model_ms}
        audit(principal,'agent',plan.kind)
    else:
        # Refresh entitlements after the slow inference step, before querying.
        with application() as db:
            current=db.execute('SELECT * FROM principals WHERE id=? AND enabled=1',(principal['id'],)).fetchone()
        if not current:
            raise HTTPException(403,detail='当前身份授权已失效。')
        output=await asyncio.to_thread(execute,dict(current),plan)
        trace.extend([{'name':'权限与计划校验','detail':'范围、字段、指标与查询形状','duration_ms':0},{'name':'只读查询','detail':f'返回 {len(output["rows"])} 条记录','duration_ms':output['duration_ms']},{'name':'结果与口径','detail':'确定性计算，无模型补造数字','duration_ms':0}])
        output.update({'trace':trace,'model':config.MODEL_ID,'model_ms':model_ms,'usage':usage})
    conversation_id=uuid4().hex
    with application() as db:
        db.execute('INSERT INTO conversations VALUES (?,?,?,?,?,?)',(conversation_id,principal['id'],question,json.dumps(plan.model_dump(),ensure_ascii=False),output['status'],datetime.now(timezone.utc).isoformat()))
