"""V2 的 Superset 身份与授权数据入口。

模型和浏览器都不能传入 Superset 账号、数据库 ID 或访问令牌。
后端从已经验证的 V2 会话选择固定业务账号，通过 Chart Data API 查询。
...
def enabled():
    """显式环境变量选源，原 SQLite 演示和隔离测试默认保持原状。"""
    backend = os.getenv("HR_V2_QUERY_BACKEND", "sqlite")
    if backend not in {"sqlite", "superset"}:
        raise HTTPException(503, "未知 V2 查询后端；拒绝自动回退")
    return backend == "superset"


def local_dir():
    return Path(os.getenv("HR_V2_SUPERSET_DIR", config.PROJECT / "integrations/superset/.local/v2"))
...
def read_local(name):
    try:
        ...
        raise HTTPException(503, "V2 Superset 尚未准备完成，请先运行集成初始化") from exc
...
def snapshot(p, refresh=False):
    ...
    with session(p) as client:
        ...
        if context["as_of"] != store.AS_OF:
            raise HTTPException(409, "V2 语义快照日与 PostgreSQL 数据版本不一致")
