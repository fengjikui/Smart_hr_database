...
DIMENSIONS = {
    'none': "'合计'", 'division': "COALESCE(v.name,'公司管理层')", 'department': 'd.name',
    'job_family': 'j.name', 'location': 'l.name', 'employment_type': 'e.employment_type',
    'relation': "CASE WHEN e.id=:viewer THEN '本人' WHEN rc.depth=1 THEN '直属下属' WHEN rc.depth>1 THEN '间接下属' ELSE '服务组织人员' END",
    'month': "SUBSTR(f.event_day,1,7)", 'day': 'f.event_day',
}
DIMENSION_LABELS={'none':'范围','division':'事业部','department':'部门','job_family':'岗位序列','location':'工作地','employment_type':'用工类型','relation':'汇报关系','month':'月份','day':'日期'}
ATTENDANCE_METRICS={'attendance_rate','late_count','late_rate','abnormal_count','approved_overtime_hours','late_departure_hours','avg_work_hours','leave_days'}
READ_TABLES={'employees','assignments','departments','positions','job_families','locations','reporting_closure','attendance_daily','overtime_requests','leave_requests','compensation'}
...
def filtered_scope(principal,plan,db):
    ...
    if plan.department:
        ...
        dept_ids=[r[0] for r in db.execute('WITH RECURSIVE tree(id) AS (SELECT ? UNION ALL SELECT d.id FROM departments d JOIN tree t ON d.parent_id=t.id) SELECT id FROM tree',(matches[0]['id'],))]
        marks=','.join('?' for _ in dept_ids)
        matching={r[0] for r in db.execute(f'SELECT employee_id FROM assignments WHERE valid_to IS NULL AND department_id IN ({marks})',dept_ids)}
        ids=sorted(set(ids)&matching)
        if not ids:
            raise HTTPException(403,detail='该查询范围不可用或超出当前身份授权。请查询自己的管理或服务范围。')
    if plan.employee_name:
        marks=','.join('?' for _ in ids) or 'NULL'
        found=rows(db,f'SELECT id FROM employees WHERE id IN ({marks}) AND (name=? OR employee_no=?)',(*ids,plan.employee_name,plan.employee_name))
        if len(found)>1:
...
def compile_query(principal,plan,db):
    ...
    if plan.metric=='avg_salary':
        if not principal['salary_aggregate']:
            raise HTTPException(403,detail='当前身份没有薪酬数据权限。你仍可查询授权范围内的人员与考勤指标。')
        if plan.kind!='metric' or plan.dimension not in ('none','division') or plan.relation!='all' or plan.department or plan.employee_name or plan.period not in ('as_of','today','this_month'):
            raise HTTPException(403,detail='薪酬仅开放当前全授权范围或事业部汇总，不支持人员筛选、历史差分和明细。')
    ...
    if (plan.metric in ATTENDANCE_METRICS or plan.kind=='attendance') and start<metadata['calendar_start']:
        raise HTTPException(422,detail=f'考勤数据从 {metadata["calendar_start"]} 开始，请缩短时间范围。')
    ids=filtered_scope(principal,plan,db)
    params={'start':start,'end':end,'begin':(date.fromisoformat(start)-timedelta(days=1)).isoformat(),'viewer':principal['employee_id'],'limit':plan.limit}
    ...
    if plan.kind=='people':
        ...
        SELECT e.employee_no AS employee_no,e.name AS name,d.name AS department,p.name AS position,
          COALESCE(manager.name,'无上级') AS manager,e.employment_type AS employment_type,l.name AS location,e.hire_date AS hire_date,
        ...
        columns=[('employee_no','员工编号'),('name','姓名'),('department','部门'),('position','岗位'),('manager','直属上级'),('employment_type','用工类型'),('location','工作地'),('hire_date','入职日期'),('relation','关系')]
    ...
    if plan.kind=='attendance':
        ...
        columns=[('employee_no','员工编号'),('name','姓名'),('department','部门'),('day','日期'),('status','状态'),('check_in','上班'),('check_out','下班'),('late_minutes','迟到分钟'),('early_minutes','早退分钟')]
        return sql,params,columns,ids,metadata
    if plan.metric in ('headcount','avg_tenure','avg_salary'):
        ...
        if plan.dimension=='month':
            ...
            while cursor<=final:
                points.append(min(final,cursor.replace(day=calendar.monthrange(cursor.year,cursor.month)[1])).isoformat())
                cursor=(cursor.replace(day=28)+timedelta(days=4)).replace(day=1)
        for i,point in enumerate(points):
            params[f'p{i}']=point
        dates=' UNION ALL '.join(f'SELECT :p{i} AS day' for i in range(len(points)))
        salary_join='JOIN compensation c ON c.employee_id=e.id AND c.valid_from<=t.day AND (c.valid_to IS NULL OR c.valid_to>t.day)' if plan.metric=='avg_salary' else ''
        value='c.monthly_base' if plan.metric=='avg_salary' else '(julianday(t.day)-julianday(e.hire_date))/365.25' if plan.metric=='avg_tenure' else '1'
        ...
        source,amount=('leave_requests','days') if plan.metric=='leave_days' else ('overtime_requests','minutes/60.0')
    ...
    return sql,params,[('label',DIMENSION_LABELS[plan.dimension]),('value',m['name']),('sample_size','涉及人数')],ids,metadata
...
def execute(principal,plan: QueryPlan,record_audit=True):
    ...
    try:
        with business() as db:
            ...
            functions={'count','sum','avg','round','coalesce','nullif','julianday','date','substr','printf'}
            def authorize(action,arg1,arg2,database,trigger):
                if action==sqlite3.SQLITE_READ:
                    return sqlite3.SQLITE_OK if arg1 in READ_TABLES and (arg1!='compensation' or principal['salary_aggregate'] and plan.metric=='avg_salary') else sqlite3.SQLITE_DENY
        ...
        if plan.kind=='metric':
            summary=f'已按{DIMENSION_LABELS[plan.dimension]}统计{definition["name"]}，共 {len(result)} 组。' if plan.dimension!='none' else f'{definition["name"]}为 {result[0]["value"] if result and result[0]["value"] is not None else "暂无可展示结果"}{definition["unit"] if result and result[0]["value"] is not None else ""}。'
        else:
            summary=f'已找到 {len(result)} 条授权范围内的{"人员" if plan.kind=="people" else "考勤异常"}记录。'
        warnings=['所有数据均为合成数据；“今天”以演示数据截止日为准。','历史人员范围按当前授权关系确定，组织分组按业务发生时任职记录归属。']
        if plan.metric in ATTENDANCE_METRICS or plan.kind=='attendance':
            warnings.append('演示日历仅采用周一至周五，不代表实际法定节假日安排。')
        if suppressed:
            warnings.append('少于 5 人的薪酬分组已隐藏；隐藏分组不参与二次合计。')
        if plan.kind!='metric' and len(result)==plan.limit:
            warnings.append(f'明细最多显示 {plan.limit} 条，请缩小范围查看其他记录。')
        output={'status':'success','summary':summary,'rows':result,'columns':[{'key':k,'label':v} for k,v in columns],'plan':plan.model_dump(),'metric':definition,'period':{'start':params['start'],'end':params['end']},'scope':{'count':len(ids),'label':'当前身份授权范围','policy_version':f'{config.POLICY_VERSION}:{principal["policy_version"]}'},'as_of':metadata['as_of'],'catalog_version':config.CATALOG_VERSION,'sql':sql,'sql_parameter_count':len(params),'duration_ms':duration,'warnings':warnings,'suppressed_groups':suppressed,'chart_type':'line' if plan.dimension in ('month','day') else 'bar' if plan.kind=='metric' and plan.dimension!='none' else 'table'}
        if record_audit:
            audit(principal,'query','allowed',plan.metric,len(ids),duration)
        return output
    except HTTPException:
        if record_audit:
            audit(principal,'query','denied',plan.metric,duration_ms=(time.perf_counter()-started)*1000)
        raise
