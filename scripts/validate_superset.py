"""真实 PostgreSQL / Superset / 当前 API 集成回归，不调用模型、不修改权限。

运行前先完成 integrations/superset 的初始化，再在项目根目录执行：
...
from backend.hr import routes as api  # noqa: E402
from backend.hr import superset_source as source  # noqa: E402
from backend.hr.api import _limits, app  # noqa: E402
from backend.hr.schema import FIELDS, Plan  # noqa: E402
...
def offline_oracle():
    ...
    rows = store.people()
    policy = store.policy()
    plans = json.loads((PROJECT / "evaluation/plans.json").read_text())["plans"]
...
def verify_http(checks, oracle):
    # 应用状态由外层临时目录初始化；客户端只验证真实路由和权限边界。
    # 只有本机模型健康状态与计划器被替换；全部认证、Superset、PG 查询均真实执行。
    client = TestClient(app, raise_server_exceptions=True)
    _limits.clear()
    try:
        checks.equal("未登录 bootstrap 被拒绝", client.get("/api/bootstrap").status_code, 401)

        def login(key):
            checks.equal(key + " HTTP 演示登录", client.post("/api/session", json={"persona_id": key}).status_code, 200)
        ...
        checks.equal("缺少 CSRF 被拒绝", client.post("/api/query", json=plan).status_code, 403)
        checks.equal("前端自由 SQL 被拒绝", client.post("/api/query", headers=headers,
                                                       json={"sql": "SELECT * FROM people"}).status_code, 422)
        checked = client.post("/api/verify", headers=headers, json=plan)
        ...
        grouped = client.post("/api/query", headers=headers, json=aggregate.model_dump())
        checks.equal("HTTP 下钻前聚合成功", grouped.status_code, 200)
        first = grouped.json()["rows"][0]
        drilled = client.post("/api/drill", headers=headers, json={
        ...
        exported = client.post("/api/export", headers=headers, json=plan)
        ...
        with patch.object(graph, "call_model", deterministic_planner):
            chatted = client.post("/api/chat", headers=headers, json={"question": "当前在职人数是多少？"})
        ...
        history_url = "/api/history/" + saved["id"]
        ...
        checks.equal("本人历史列表包含本次查询", saved["id"] in [r["id"] for r in client.get("/api/history").json()], True)
        ...
        checks.equal("跨用户历史列表不含他人记录", saved["id"] in [r["id"] for r in client.get("/api/history").json()], False)
        checks.equal("员工 HTTP 导出被拒绝", client.post("/api/export", headers=employee_headers, json=plan).status_code, 403)
        checks.equal("员工 HTTP 合同列被拒绝", client.post("/api/query", headers=employee_headers,
            json={"kind": "people", "columns": ["contract_end_date"]}).status_code, 403)
        stale = TestClient(app)
        try:
            stale.cookies.set(auth.COOKIE, old_token)
            checks.equal("身份切换撤销旧会话", stale.get("/api/bootstrap").status_code, 401)
        finally:
            stale.close()
    finally:
        client.close()
...
def run(report_path):
    ...
    try:
        ...
        with TemporaryDirectory(prefix="hr-superset-validation-") as directory, ExitStack() as stack:
            stack.enter_context(patch.dict(os.environ, {"HR_QUERY_BACKEND": "superset"}))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--report", type=Path, default=PROJECT / "reports/superset-integration.json")
