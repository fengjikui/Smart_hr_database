...
不能读取本地 people.sqlite 重新计算。编译检查则沿用 V2 独立隔离的权限样本。
"""
...
from backend.hr import config, v2
from backend.hr.v2 import auth, query, store, superset_query
from backend.hr.v2.schema import FIELDS, METRICS, Plan
...
def source_double(monkeypatch, snapshot, results):
    ...
    monkeypatch.setitem(sys.modules, "backend.hr.v2.superset_source", double)
    monkeypatch.setattr(v2, "superset_source", double, raising=False)
    return calls
