...
问数页使用独立滚动的对话区与底部输入框。用户问题靠右，回答和相关表格顺序展示；旧回答保留摘要，按需展开表格及节点记录。输入框默认单行，总高约62px；两行时自动增高，长内容最多104px输入高度后内部滚动。24px圆角与轻微阴影，框内32px箭头发送；空值时同行显示提示，输入后隐藏。追问上下文压缩为一条细行，可点叉清除。Enter发送、Ctrl+Enter在光标处换行，中文输入法组合输入期间不触发发送。条件、核验、控件与表格采用8–14px圆角，浅红色统一信息面板，状态仍有文字说明。
