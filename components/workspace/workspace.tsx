'use client';
/**
 * 页面总装配：HRWorkspace 管身份和数据版本，Workspace 管当前一次聊天/核验交互。
 * 首页与 /debug 共用身份壳，所有页面都使用同一套查询协议。
 * 所有业务请求经 types.ts → 同源 /api → FastAPI，浏览器不直连 PostgreSQL/Superset。
 */
...
export default function HRWorkspace({ debug = false }: { debug?: boolean }) {
  ...
  const refresh = useCallback(async (persona?: string) => {
    ...
    try {
      ...
      let response = await fetch('/api/bootstrap');
          ...
          <div className="d-scope-note">
            ...
            <details>
              ...
              <div className="d-account-details">
                ...
                <span>
                  {boot.query_backend === 'superset_mcp_catalog_rest_query'
                    ? 'MCP 目录 · REST 查询'
