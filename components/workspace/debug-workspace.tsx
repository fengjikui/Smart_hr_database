'use client';
/**
 * 独立节点调试页：读取已保存历史的 trace，展示每一步真实记录的输入、输出和耗时。
 * 本页不重新触发模型推理；缺失 trace 时明确显示未记录，不能补造一条执行链。
 * 调试只读取已保存记录，不能绕过当前身份和权限指纹。
 */
  ...
  return (
    <div className="d-trace-page">
      <aside className="d-trace-history" aria-label="节点调试导航与查询记录">
        <Link href="/" className="d-trace-back">
          <ArrowLeft size={16} />
          返回问数
        </Link>
        <h1>
        ...
        <nav aria-label="选择查询记录" className="d-trace-records">
          {listError ? (
            ...
            history.map((item) => (
              <Link
                key={item.id}
                href={`/debug?run=${encodeURIComponent(item.id)}`}
                prefetch={false}
    ...
    <>
      <header className="d-trace-run-header">
        ...
        <div className="d-trace-meta">
          ...
          {run.parent_id && (
            <Link
              href={`/debug?run=${encodeURIComponent(run.parent_id)}`}
              prefetch={false}
            >
              查看上一轮查询
            </Link>
