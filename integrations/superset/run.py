"""宿主机入口：export 导出当前，setup 建立隔离演示，status 只读盘点。
...
def container_command(script, *args):
    ...
    env = dict(os.environ, DOCKER_HOST=os.environ.get(
        "HR_DOCKER_HOST", f"unix://{ROOT.home()}/.colima/hr-superset/docker.sock"))
    compose = ["docker-compose"] if shutil.which("docker-compose") else ["docker", "compose"]
    lookup = subprocess.run([*compose, "-f", str(ROOT / "compose.yaml"), "ps", "-q", "superset"],
...
def setup(sync_data=False):
    ...
    print(f"独立演示已准备：数据库 {manifest['database']}，{manifest['row_count']} 人，"
