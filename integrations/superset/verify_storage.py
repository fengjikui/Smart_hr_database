"""只读核验 当前 迁移与权限结构；不添加、删除、改写任何授权对象。
...
import psycopg2
from setup import password, pg, read_fixture
...
def main():
    ...
    try:
        with conn.cursor() as cur:
            ...
            check("全量宽表列顺序与当前 相同", fields, [f["id"] for f in fixture["fields"]])
    ...
    check("8 个自定义角色", sorted(r.name for r in roles), sorted(role_names))
    ...
    check("7 个 独立账号", sorted(u.username for u in users), sorted(usernames))
    ...
    check("3 条 RLS", sorted(r.name for r in rules), sorted(expected_rules))
    for rule in rules:
