"""容器内初始化完整 V2 演示；由宿主机 run.py 调用，不打印任何密码。
...
def prepare_database(fixture, sync_data=False):
    ...
    with conn, conn.cursor() as cur:
        ...
        if not cur.fetchone()[0]:
            raise ValueError("V2 数据存在管理环、主管孤儿或 HRBP 孤儿，本次导入回滚")
    return conn
...
def create_chart(db, Slice, Dashboard, admin, table, level):
    title = "V2 · " + ("人员与汇报范围" if level == "public" else "合同字段权限")
...
def prepare_superset(sync_data=False):
    ...
    admin = sm.find_user(username="v2_setup_admin") or sm.add_user(
        "v2_setup_admin", "V2", "配置管理员", "v2_setup_admin@example.invalid",
    ...
    for name, members, clause in [
        ...
        if rule is None:
            rule = RowLevelSecurityFilter(name=name, clause=clause, filter_type=RowLevelSecurityFilterType.BASE,
                ...
                description="V2 登录用户隔离；业务递归关系在 hr_v2.v2_auth 视图计算，不能由问题指定查看人。")
            db.session.add(rule)
        db.session.commit()
    ...
    for persona in fixture["personas"]:
        ...
        if user is None:
            user = sm.add_user(username, "V2", persona["name"], username + "@example.invalid", roles, password(username))
    ...
    unmapped = sm.find_user(username="v2_unmapped") or sm.add_user("v2_unmapped", "V2", "未映射反例",
