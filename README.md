...
- 六个实际可用页面、移动窄屏适配、查询忙碌/模型离线/空结果/拒绝状态、API限制与审计。
...
已验证48项自动化测试通过；本机模型场景20/20通过，这不表示任意问法准确率100%。具体记录与测试边界见 [测试说明](docs/TESTING.md)、[模型报告](reports/model-evaluation.json)、[性能报告](reports/performance.json)。GitHub CI与手动发布打包工作流已提供；尚未指定远程仓库和内网部署目标，因此未远程执行或发布。
